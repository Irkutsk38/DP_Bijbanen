...
AddEventHandler('esx:playerLoaded', function(xPlayer)
    PlayerData = xPlayer
    TriggerServerEvent('esx_jobssecondary:giveBackCautionInCaseOfDrop')
    refreshBlips()
end)

AddEventHandler('esx_jobssecondary:publicTeleports', function(position)
...
AddEventHandler('esx_jobssecondary:action', function(job2, zone)
  menuIsShowed = true
  ...
  elseif zone.Type == "work" then
    ...
    if IsPedInAnyVehicle(playerPed, 0) then
      ...
      TriggerServerEvent('esx_jobssecondary:startWork', zone.Item)
    end
  elseif zone.Type == "vehspawner" then
    TriggerServerEvent('esx_jobssecondary:requestPlayerData', 'refresh_bank_account')
    local spawnpt = nil
    ...
    TriggerServerEvent('esx_jobssecondary:setCautionInCaseOfDrop', caution)
            ...
            if IsPedInAnyVehicle(playerPed, 0) then
              ...
              for i=1, #myPlate, 1 do
                if (myPlate[i] == plate) and (playerPed == driverPed) then
                  TriggerServerEvent('esx_jobssecondary:caution', "give_back", cautionVehicleInCaseofDrop, 0, 0)
                  ...
                  if vehicleObjInCaseofDrop.HasCaution then
                    ...
                    TriggerServerEvent('esx_jobssecondary:setCautionInCaseOfDrop', 0)
                  end
...
-- #########################
AddEventHandler('esx_jobssecondary:hasExitedMarker', function(zone)
  TriggerServerEvent('esx_jobssecondary:stopWork')
  hintToDisplay = "no hint to display"
  menuIsShowed = false
  hintIsShowed = false
  isInMarker = false
...
function spawncar(spawnPoint, vehicle)
  hintToDisplay = "no hint to display"
  hintIsShowed = false
  TriggerServerEvent('esx_jobssecondary:caution', "take", cautionVehicleInCaseofDrop, spawnPoint, vehicle)
end

RegisterNetEvent('esx_jobssecondary:spawnJobVehicle')
AddEventHandler('esx_jobssecondary:spawnJobVehicle', function(spawnPoint, vehicle)
  local playerPed = GetPlayerPed(-1)
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlJustReleased(0, Keys["E"]) and isInPublicMarker then
      TriggerEvent('esx_jobssecondary:publicTeleports', position)
    end
      ...
      if zones ~= nil then
        ...
        if IsControlJustReleased(0, Keys["E"]) and not menuIsShowed and isInMarker then
          if onDuty or zone.Type == "cloakroom" or PlayerData.job2.name == "reporter" then
            TriggerEvent('esx_jobssecondary:action', job, zone)
          end
        end
        ...
        if not isInMarker and hasAlreadyEnteredMarker then
          ...
          TriggerEvent('esx_jobssecondary:hasExitedMarker', zone)
        end
      end
    end
  end
    ...
    if vehicleInCaseofDrop ~= nil then
      if onDuty and IsVehicleModel(vehicleInCaseofDrop, vehicleHashInCaseofDrop) then
        ...
        if vehicleOldHealthInCaseofDrop ~= vehicleHealth then
          ...
          TriggerServerEvent('esx_jobssecondary:setCautionInCaseOfDrop', cautionValue)
        end
      end
    end
  end
